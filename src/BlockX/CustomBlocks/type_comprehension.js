...
Blockly.COMP_SETTINGS = {
  'ListComp': {start: '[', end: ']', color: 160},
  'SetComp': {start: '{', end: '}', color: 170},
  'GeneratorExp': {start: '(', end: ')', color: 180},
  'DictComp': {start: '{', end: '}', color: 190},
};
...
['ListComp', 'SetComp', 'GeneratorExp', 'DictComp'].forEach(function(kind) {
  Blockly.Blocks[kind] = {
    ...
    init: function() {
      ...
      this.setColour(Blockly.COMP_SETTINGS[kind].color);
      this.itemCount_ = 3;
      const input = this.appendValueInput('ELT')
          .appendField(Blockly.COMP_SETTINGS[kind].start);
      if (kind === 'DictComp') {
        input.setCheck('DictPair');
      }
      this.appendDummyInput('END_BRACKET')
          .appendField(Blockly.COMP_SETTINGS[kind].end);
      this.updateShape_();
      this.setOutput(true);
      this.setMutator(new Blockly.Mutator(['comp_create_with_for',
        'comp_create_with_if']));
    },
    ...
    decompose: function(workspace) {
      ...
      for (let i = 1; i < this.itemCount_; i++) {
        ...
        let createName;
        if (generator.targetConnection
            .getSourceBlock().type === 'comprehensionIf') {
          createName = 'comp_create_with_if';
        } else if (generator.targetConnection
            .getSourceBlock().type === 'comprehensionFor') {
          createName = 'comp_create_with_for';
        } else {
          throw Error('Unknown block type: ' + generator.targetConnection
              .getSourceBlock().type);
        }
    ...
    compose: function(containerBlock) {
      ...
      for (let i = 1; i < this.itemCount_; i++) {
        const connection = this.getInput('GENERATOR' + i)
      ...
      for (let i = 1; i < this.itemCount_; i++) {
        ...
        if (!connections[i]) {
          ...
          itemBlock.initSvg();
          this.getInput('GENERATOR' + i).connection
